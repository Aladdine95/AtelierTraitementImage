...
void* SobelEDThread(void* params){
  paramSobel*param = (struct paramSobel *)params;
  if( param->limit  >255 || param->limit<0)
    param->limit=255;
  float sumX,sumY;
  for(int y=1;y< param->frame->cols-1;y++){
    for(int x=1;x< param->frame->rows-1;x++){
      sumX=0;
      sumY=0;
      sumX = param->frame->at<uchar>(Point(y,x+1))-param->frame->at<uchar>(Point(y,x-1));
      sumY = param->frame->at<uchar>(Point(y+1,x))-param->frame->at<uchar>(Point(y-1,x));

      if(sqrt(pow(sumX,2)+pow(sumY,2))>param->limit){
        param->out->at<uchar>(Point(y,x)) = 255;
      }

      else{
        param->out->at<uchar>(Point(y,x)) = 0;
      }
    }
  }
}
...
void* simpleHoughThread(void *params){
  paramHough *param = (struct paramHough*)params;
  int channels = param->frame->channels();
  int nRows = param->frame->rows;
  int nCols = param->frame->cols * channels;
  //const uchar* image = frame.ptr();
  int step = (int)param->frame->step;
  int stepacc = (int)param->acc->step;
  if (param->frame->isContinuous())
  {
      nCols *= nRows;
      nRows = 1;
  }
  ...
  int i,j;
  double rho;
  for( i = 0; i < param->frame->rows; i++ ){
    for( j = 0; j < param->frame->cols; j++ ){
      if(param->frame->data[i * step + j]!=0){
        for(int theta=0;theta<180; theta+=thetaStep){
          rho = j*cos((double)theta*PI/180)+i*sin((double)theta*PI/180);
          if(rho!=0)
            param->acc->at<ushort>(Point(cvRound(rho) ,(int)cvRound(theta/thetaStep)))+=1;
        }
      }
    }
  }
  ...
  cv::Point min_loc_old, max_loc_old;
  double min, max;
  cv::minMaxLoc(*(param->acc), &min, &max, &min_loc_old, &max_loc_old);

  Point pt1, pt2;
  double a ,b;
  double x0, y0;
  double theta;
  param->acc->data[max_loc_old.y * stepacc +max_loc_old.x]=0;
  for(int i=0;i<40;i++){
    cv::minMaxLoc(*(param->acc), &min, &max, &min_loc, &max_loc);
    if(abs(max_loc_old.x-max_loc.x)>5 || abs(max_loc_old.y-max_loc.y)>5){ //might be interesting to use that ....
      theta = (double)max_loc.y*thetaStep;
      a = cos(theta*PI/180); //compute hough inverse transform from polar to cartesian
      b = sin(theta*PI/180);
      ...
      pt2.x = cvRound(x0 - 1000*(-b)); //compute second point
      pt2.y = cvRound(y0 - 1000*(a));
      line( *(param->f), pt1, pt2, Scalar(0,0,255), 3, LINE_AA);
      param->acc->at<ushort>(Point(max_loc.x ,max_loc.y))=0;
      max_loc_old.x = max_loc.x;
      max_loc_old.y = max_loc.y;
    }
    else{
...
int main(int argc, char** argv)
{
  ...
  printf("setup\n");
    ...
    for(;;)
    {
        ...
        paramHough *parameterHough = (paramHough*)malloc(sizeof(parameterHough));
        parameterHough->frame = &sobel;
        parameterHough->acc = &acc;
        parameterHough->f = &frame;
        paramSobel *parameterSobel =  (paramSobel*)malloc(sizeof(parameterSobel));
        parameterSobel->frame =  &grayscale;
        parameterSobel -> out = &sobel;
        parameterSobel -> limit = SOBEL_LIMIT;

        //pthread_create(&thread_1, NULL,SobelEDThread,parameterSobel);
        pthread_create(&thread_2,NULL, simpleHoughThread,parameterHough);
        ...
        imshow( "Sobel Frame", *(parameterHough->f));  // show filter image
        imshow("Acc Frame", acc);
